...
class AddPostActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<ImageView>(R.id.save_new_post_btn).setOnClickListener {
            uploadImage()
        }
            CropImage.activity()
                .setAspectRatio(2,1)
    ...
    private fun uploadImage() {
        when {
            ...
            TextUtils.isEmpty(findViewById<EditText>(R.id.description_post).text.toString()) -> {
                Toast.makeText(this,"please write description first", Toast.LENGTH_LONG).show()
            }
            else -> {
                val progressDialog = ProgressDialog(this)
                progressDialog.setTitle("Adding New Post")
                progressDialog.setMessage("Please wait,we are adding your posts...")
                progressDialog.show()
